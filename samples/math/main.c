...
int main()
{
    ...
    xnmeaPOS pos[NUM_POINTS], pos_moved[NUM_POINTS][2];
    ...
    for(it = 0; it < NUM_POINTS; ++it)
    {
        ...
        result = xnmea_parse(&parser, buff[it], (int)strlen(buff[it]), &info);
        xnmea_info2pos(&info, &pos[it]);
    }
    ...
    for(it = 0; it < NUM_POINTS; ++it)
    {
        dist[it][0] = xnmea_distance(&pos[0], &pos[it]);
        dist[it][1] = xnmea_distance_ellipsoid(
    ...
    for(it = 0; it < NUM_POINTS; ++it)
    {
        result[0] = xnmea_move_horz(&pos[0], &pos_moved[it][0], azimuth[it][0], dist[it][0]);
        result[1] = xnmea_move_horz_ellipsoid(
    ...
    for(it = 0; it < NUM_POINTS; ++it)
    {
        printf(
            ...
            it, pos[it].lat, pos[it].lon, xnmea_radian2degree(pos[it].lat), xnmea_radian2degree(pos[it].lon)
            );
    }
